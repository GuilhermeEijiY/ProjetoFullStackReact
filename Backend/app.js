...
setupDatabase() 
    .then(async () => {
        try {
            // Tenta criar usuários iniciais se eles não existirem
            ...
            // O código 11000 geralmente indica erro de duplicidade (usuário já existe),
            if (err.code !== 11000) {
                console.error("Erro ao criar usuários iniciais:", err);
            }
        }

        // Inicia o servidor Express APENAS DEPOIS que tudo acima foi feito
        app.listen(PORT, () => {
